...
class IconBrowser extends React.Component {
  ...
  getMatchingIcons(filter) {
    if (!filter) return this.props.types

    const search = filter.toLowerCase()

    return Object.keys(this.props.aliases)
      .filter(alias => fuzzysearch(search, alias))
      .map(alias => this.props.aliases[alias])
      .filter((value, index, arr) => arr.indexOf(value) === index)
      .sort()
  }

  filterIconsForSection(section, icons) {
    const sectionIcons = icons.filter(
      icon => !!this.props.processedSections[section][icon]
    )
    ...
    return (
      <Browser>
        <RowLayout gutter="large">
          ...
          />
          <Rows gutter="large">
            {Object.keys(this.props.processedSections).map(section =>
...
export default function() {
  ...
  `)
  const { types, aliases } = data.allIcons.edges[0].node
  const _aliases = aliases.reduce((result, item) => {
    result[item.name] = item.value
    return result
  }, {})
  const processedSections = (() => {
    const uniqueIcons = [...new Set(Object.values(_aliases))]
    const uniqueAliasedIcons = Object.keys(sections).reduce(
      (prev, current) => [...prev, ...Object.keys(sections[current])],
      []
    )
    const notAliasedIcons = uniqueIcons.filter(
      item => !uniqueAliasedIcons.includes(item)
    )
