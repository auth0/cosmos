...
variants['isologotype'] = ({ color, variant }) => (
...
variants['isotype'] = ({ color, size, variant }) => (
  <svg height={sizes[variant][size].height} width={sizes[variant][size].width} viewBox="0 0 29 32">
    <g transform="translate(-100.000000, -88.000000)" fill={colors[variant][color]}>
...
const Logo = ({ size, color, variant }) => (
  <Logo.Element size={size} variant={variant}>
    {variants[variant]({ color, size, variant })}
  </Logo.Element>
)

Logo.Element = styled.span`
  display: inline-block;
  height: ${props => sizes[props.variant][props.size].height}px;
  width: ${props => sizes[props.variant][props.size].width}px;
  svg {
    display: block;
  }
`
...
Logo.propTypes = {
  ...
  /** Defines whether to show only the isotype or the complete isologotype */
  variant: PropTypes.oneOf(['isotype', 'isologotype'])
}
